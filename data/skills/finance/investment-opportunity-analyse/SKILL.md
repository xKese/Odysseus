...
1. Klassifiziere die Opportunitaet in eine der vier Asset-Klassen: (a) Private Equity / Venture-Fonds, (b) Immobilien (direkt oder Immobilienfonds), (c) direkte Unternehmensbeteiligung, (d) Private Debt oder Infrastruktur. Bei Hybriden die dominante Klasse waehlen und den Hybridcharakter vermerken. Frage nach gewuenschter Ausfuehrlichkeitsstufe (Kurzbewertung / Detailanalyse / Vergleich mehrerer Opportunitaeten) UND Recherchemodus (siehe Skill `recherche-modus`): Schnellanalyse (nur Upload-Dokumente + gezielter `web_fetch`) oder Tiefenanalyse (zusaetzlich `trigger_research`). Wenn der Nutzer den Modus nicht nennt, nachfragen — niemals ungefragt einen Deep-Research-Lauf starten. In dieser Domaene validiert Deep Research vor allem: den GP-/Initiator-Track-Record unabhaengig, Reputations-/Litigation-/Negativ-Recherche, Marktbenchmarks (z.B. Cap Rates der Lage, PE-Benchmark-Renditen) und Hinweise auf Related-Party-Konstellationen. Falls das Pitchdeck noch fehlt, fordere es zum Upload an.
2. Extrahiere die allgemeinen Stammdaten plus das asset-klassen-spezifische Kennzahlen-Modul (siehe body_extra) aus dem Pitchdeck via `read_file`. Gezielte `web_fetch`-Abrufe auf konkret benannte oeffentliche Quellen sind in beiden Modi erlaubt. NUR im Tiefenmodus zusaetzlich `trigger_research` mit Quellenpflicht fuer den Anbieter-/GP-/Initiator-Hintergrund und dessen Track Record; den fertigen Bericht ueber `manage_research` lesen und einarbeiten. Deep-Research-Befunde mit Marker `(Deep Research: <Quelle>, <Stichtag>)` kennzeichnen und vom Pitchdeck-Inhalt trennen. Im Schnellmodus KEIN `trigger_research`. Niemals Zahlen schaetzen.
